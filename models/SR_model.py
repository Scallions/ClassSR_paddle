...
class SRModel(BaseModel):
    ...
    def get_current_visuals(self, need_GT=True):
        ...
        out_dict['LQ'] = self.var_L.detach()[0].as_type('float')#().cpu()
        out_dict['rlt'] = self.fake_H.detach()[0].as_type('float')#().cpu()
        if need_GT:
            out_dict['GT'] = self.real_H.detach()[0].as_type('float')#().cpu()
        return out_dict
